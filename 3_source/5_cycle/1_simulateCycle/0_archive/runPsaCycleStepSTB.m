...
%             implemented here is called the method of lines. In
%             particular, this .m function utilizied cvodes.m from SUNDIALS
%             TOOLBOX (STB) developed from Lawrence Livermore National
%             Laboratory (LLNL).
...
%     funcEve    = params.funcEve{nS};
    ...
    funcRhs = @(t,y,params)defineRhsFuncSTB(t,y,params);
    %---------------------------------------------------------------------%
...
%     %---------------------------------------------------------------------%
    ...
    %---------------------------------------------------------------------%
    %Initialize the important options for CVode.m
    
    %Get the user data from the data structure
    options = CVodeSetOptions('UserData',params);
    ...
    %---------------------------------------------------------------------%
    %Initialize the CVode.m function with the right hand side function,
    %initial conditions, and the problem data
    
    %Initialize the CVode.m
    CVodeInit(funcRhs,'BDF','Newton',tDom(1),iStates',options);
    %---------------------------------------------------------------------%
    ...
    %Get the time points where we need to obtain the function output
    tSpan = linspace(tDom(1),tDom(2),nTiPts+1);
    
    %Call CVode.m
    [status,stTime,stStates] = CVode(tSpan(2:end),'Normal');        
    
    %Set the memory used by CVode.m free
    CVodeFree;
    
    %Transpose the state solution matrix
    stStates = stStates';
    %---------------------------------------------------------------------%
...
%     end        
    ...
    %Check if CVode.m succeeded in obtaining the solutions
    if status == -1
        
       %Print out the error message
        msg = 'CVode.m did not obtain the numerical solution';
        msg = append(funcId,': ',msg);
        error(msg); 
        
    end
    
    %Set the event flag equal to zero for now (this is time driven mode)
    flags.event = 0;
    %---------------------------------------------------------------------%     
